...
def create_bar_plot(rtt_array, num_hops):
    ...
    bars = plt.bar(hops, rtts, color='purple')
    ...
    plt.savefig("C:/Users/franc/OneDrive/Área de Trabalho/projrede/projredes/static/projredesapp/images/graph.png", dpi=300)
    plt.show()

def information():

    rtt_float_array = list(map(float,rtt_array))

    min_rtt = min(rtt_float_array)
    max_rtt = max(rtt_float_array)
    mean_rtt = numpy.mean(rtt_float_array)

    list_info.append(min_rtt)
    list_info.append(max_rtt)
    list_info.append(mean_rtt)
...
def function_graph():
    return create_bar_plot(rtt_array, num_hops)
